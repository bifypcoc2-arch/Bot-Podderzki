    ...
    while True:
        ...
        for user_id in user_ids:
            try:
                ...
                logger.exception("Ошибка обновления параметров у %s", user_id)

        logger.info("Обновлены параметры у %s питомцев", len(user_ids))
...
async def supervise(name: str, factory):
    ...
    вроде работает, а питомцы больше не голодают и бэкапы не снимаются.
    Задержка растёт до 5 минут, чтобы сломанная насовсем задача не
    крутилась в пустую.
    """
...
async def main():
    ...
    tasks = [
        asyncio.create_task(supervise("деградация питомцев", parameter_decay_task)),
